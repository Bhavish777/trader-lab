...
def _build_priced_holding(
    holding: Holding,
    current_price: float,
    price_source: str,
) -> dict:
    """Create a priced holding response from a holding and current price."""
...
def price_holding(holding: Holding) -> dict:
    """Attach market price and unrealized P&L to one holding."""
    try:
        quote = get_latest_quote(holding.symbol)

        return _build_priced_holding(
            holding=holding,
            current_price=quote["price"],
            price_source=quote["source"],
        )
    except Exception:
        # Keep the portfolio usable even when market data is unavailable.
        return _build_priced_holding(
            holding=holding,
